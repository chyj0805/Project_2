...
payroll_url = 'https://www.alphavantage.co/query?function=NONFARM_PAYROLL&apikey=4FHTO2GAT3NL1EZ8'
payroll_r = requests.get(payroll_url)
payroll_data = payroll_r.json()

pay_data = payroll_data['data']

# Create a DataFrame from the 'data' list
payroll_df = pd.DataFrame(pay_data)

# Convert the 'date' column to datetime format
payroll_df['date'] = pd.to_datetime(payroll_df['date'])

# Filter the DataFrame for dates from the year 2000 onwards
payroll_df = payroll_df[payroll_df['date'].dt.year >= 2000]

# Set the 'date' column as the DataFrame index
payroll_df.set_index('date', inplace=True)

# Resample the DataFrame to have daily rows until a new month changes
resampled_df = payroll_df.resample('D').ffill()

# Reset the index to make 'date' a regular column
resampled_df.reset_index(inplace=True)

# Rename the 'value' column to 'US Payroll'
resampled_df.rename(columns={'value': 'US Payroll'}, inplace=True)
...
class StockModelTrainerPredictor:
    ...
    # Fetch Non Farm Payroll Data
    def fetch_non_farm_payroll_data(self):
        payroll_url = 'https://www.alphavantage.co/query?function=NONFARM_PAYROLL&apikey={self.api_key}'
        payroll_r = requests.get(payroll_url)
        payroll_data = payroll_r.json()
    ...
    def get_combined_data(self, interval="daily", series_type="close"):
        ...
        # Merge Non Farm Payroll Data
        non_farm_payroll_df = self.fetch_non_farm_payroll_data()
        ...
        combined_df = pd.merge(combined_df, non_farm_payroll_df, on='Date', how='left')
    ...
    def preprocess_data(self):
        ...
        x = self.data[["Open", "High", "Low", "Volume", "MACD_Signal", "MACD_Hist", "MACD", "Federal Funds Rate", "Unemployment Rate"."US Payroll"]].to_numpy()
    ...
    def predict_next_60_days(self):
        ...
        # Data preprocessing
        new_x = self.data[["Open", "High", "Low", "Volume", "MACD_Signal", "MACD_Hist", "MACD", "Federal Funds Rate", "Unemployment Rate","US Payroll"]].to_numpy()
        scaler_x = MinMaxScaler()
