# -*- coding: utf-8 -*-
"""Energy_Predicition_Class.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ATqRw9AA-cvGx54m2WJZHx6GpMP3lJfU
...
api_url = f"https://api.eia.gov/v2/natural-gas/stor/type/data/?frequency=monthly&data[0]=value&facets[series][]=N5070US2&start=2000-01&sort[0][column]=period&sort[0][direction]=desc&offset=0&length=5000&api_key={api_key}"
...
if response.status_code == 200:
    ...
    if "data" in json_data["response"]:
        ...
        gas_storage_df = pd.DataFrame(data_list)

        # Convert 'period' column to datetime format
        gas_storage_df['period'] = pd.to_datetime(gas_storage_df['period'])

        # Reorder the DataFrame by sorting it in ascending order based on the 'period' column
        gas_storage_df = gas_storage_df.sort_values(by='period')

        # Rename the 'period' column to 'Date'
        gas_storage_df = gas_storage_df.rename(columns={'period': 'Date'})

        # Set 'Date' as the index
        gas_storage_df.set_index('Date', inplace=True)

        # Resample the data to fill in daily rows using forward-fill (ffill)
        gas_storage_df = gas_storage_df.resample('D').ffill()

        # Reset the index
        gas_storage_df.reset_index(inplace=True)

        # Rename the 'value' column to 'Gas Storage Value' (you can replace 'Gas Storage Value' with your desired name)
        gas_storage_df = gas_storage_df.rename(columns={'value': 'Nat Gas Net Withdrawls'})

        # Print the updated DataFrame
        print(gas_storage_df)
    else:
...
class StockModelTrainerPredictor:
    ...
    def get_combined_data(self, interval="daily", series_type="close"):
        ...
        # Add Nat Gas Net Withdrawls
        gas_storage_df['Date'] = pd.to_datetime(gas_storage_df['Date'])
        combined_df = pd.merge(combined_df, gas_storage_df[['Date', 'Nat Gas Net Withdrawls']], on='Date', how='left')
