# -*- coding: utf-8 -*-
"""Copy of StockModelTrainerPredictor_class.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Nr2OXq068evo8TnXzFaqqqDNkgrr2aCd
"""
...
#US Natural Gas Inventory In Million cubic feet MMcf

# Define your API key (replace 'your_api_key_here' with your actual API key)
api_key = "S8c7DuNJmAmuaTEB9dogKmXOgS7AVaGTZcprS8JL"

# Define the API URL (with parameters included)
api_url = f"https://api.eia.gov/v2/natural-gas/stor/sum/data/?frequency=monthly&data[0]=value&facets[series][]=NGM_EPG0_SAT_R48_MMCF&start=2000-01&sort[0][column]=period&sort[0][direction]=desc&offset=0&length=5000&api_key={api_key}"
...
if response.status_code == 200:
    ...
    if "data" in json_data["response"]:
        data = json_data["response"]["data"]  # Extract the 'data' field
        ...
        nat_inventory_df = pd.DataFrame(data_list)


        # Convert 'period' column to datetime format
        nat_inventory_df['period'] = pd.to_datetime(nat_inventory_df['period'])

        # Set the 'period' column as the index
        nat_inventory_df.set_index('period', inplace=True)

        # Create a date range for the entire period
        date_range = pd.date_range(start=nat_inventory_df.index.min(), end=nat_inventory_df.index.max(), freq='D')

        # Create a daily DataFrame with the same index as 'date_range' and NaN values
        daily_df = pd.DataFrame(index=date_range)

        # Merge the original monthly data with the daily DataFrame and forward-fill the missing values
        daily_df = daily_df.merge(nat_inventory_df[['value']], how='left', left_index=True, right_index=True)
        daily_df['value'].fillna(method='ffill', inplace=True)

        # Reset the index to include the 'period' column
        daily_df.reset_index(inplace=True)
        daily_df.rename(columns={'index': 'period'}, inplace=True)

        # Rename the 'value' column to 'Nat Gas Vol MMcf'
        daily_df.rename(columns={'value': 'Nat Gas Vol MMcf'}, inplace=True)
...
class StockModelTrainerPredictor:
    ...
    def get_combined_data(self, interval="daily", series_type="close"):
        ...
        # Add 'Nat Gas Vol MMcf' column from daily_df
        daily_df['Date'] = pd.to_datetime(daily_df['Date'])
        combined_df = pd.merge(combined_df, daily_df[['Date', 'Nat Gas Vol MMcf']], on='Date', how='left')
    ...
    def preprocess_data(self):
        ...
        x = self.data[["Open", "High", "Low", "Volume", "MACD_Signal", "MACD_Hist", "MACD", "Federal Funds Rate", "Unemployment Rate","Nat Gas Vol MMcf"]].to_numpy()
    ...
    def predict_next_60_days(self):
        ...
        # Data preprocessing
        new_x = self.data[["Open", "High", "Low", "Volume", "MACD_Signal", "MACD_Hist", "MACD", "Federal Funds Rate", "Unemployment Rate","Nat Gas Vol MMcf"]].to_numpy()
        scaler_x = MinMaxScaler()
